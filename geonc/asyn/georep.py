...
class GeorepNC(GeoRequests):
    ...
    async def get_info(self, adresse: str = "") -> GeoClass:
        ...
        fin = await self.arequest(method="GET", endpoint=f"/api/parcel/fts?{data}", payload=self.payload, headers=self.request_header)
        fin = fin.json
        
        return GeoObject(fin)

    async def get_adresse_list(self, number: str = "", street: str = "", nic: str = "") -> list[GeoClass]:
        """
        parameters :
        ...
        dico: GeoClass = await self.get_info(adresse)
    ...
    async def get_adresse(self, number: str = "", street: str = "", nic: str = "") -> list[GeoClass]:
        """
        parameters :
    ...
    async def get_nic(self, nic: str) -> list[GeoClass]:
        """
        parameters :
            nic : the nic (numero d'Inventaire Cadastral)
        ...
        data = urlencode(data).replace("%27", "%22").replace("+", "%20").replace("%20", "", 1)
        res = await self.arequest(method="GET", endpoint=f"/api/parcel?{data}", payload=self.payload, headers=self.nic_header)
        res = res.json

        return GeoObject(res)

    async def get_coords(self, x: float = None, y: float = None, nic: str = None) -> GeoClass:
        if x and y:
            data: dict = {
                "f": "json",
